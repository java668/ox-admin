package com.java668.oxadmin.modules.generator.utils;
...
public class VelocityUtils {
     ...
     * @return 模板列表
     */
    public static VelocityContext prepareContext(Table genTable) {
    ...
    public static void setTreeVelocityContext(VelocityContext context, Table genTable) {
        String options = genTable.getOptions();
        JSONObject paramsObj = JSON.parseObject(options);
    ...
    public static void setSubVelocityContext(VelocityContext context, Table genTable) {
        Table subTable = genTable.getSubTable();
     ...
     * @return 返回需要导入的包列表
     */
    public static HashSet<String> getImportList(Table genTable) {
        List<TableColumn> columns = genTable.getColumns();
        Table subGenTable = genTable.getSubTable();
     ...
     * @return 返回字典组
     */
    public static String getDicts(Table genTable) {
        List<TableColumn> columns = genTable.getColumns();
        Set<String> dicts = new HashSet<String>();
        addDicts(dicts, columns);
        if (StringUtils.isNotNull(genTable.getSubTable())) {
            List<TableColumn> subColumns = genTable.getSubTable().getColumns();
     ...
     * @return 展开按钮列序号
     */
    public static int getExpandColumn(Table genTable) {
        String options = genTable.getOptions();
        JSONObject paramsObj = JSON.parseObject(options);
        String treeName = paramsObj.getString(GenConstants.TREE_NAME);
        int num = 0;
        for (TableColumn column : genTable.getColumns()) {
            if (column.isList()) {
                num++;
